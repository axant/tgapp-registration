...
The hooks that can be used with TurboGears2 *register_hook* are:
    ...
    * **registration.before_registration_form(arguments)** -> Runs before rendering the form. Can be used to insert preconditions to limit the registration for example just to invited users. Tou can fill the form inserting data into arguments, you might want to put a json serialized dictionary into arguments['params'] that is an HiddenField in the form
